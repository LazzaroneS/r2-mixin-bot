"""A language translator and detector by google translator
Usage:
::
    >>> from google import translator as t
    >>> t.translate('hello', source_lang='zh-CN', dest_lang='en')
    >>> t.detect("你好")
"""
...
BASE_URL = "http://translate.google.com/translate_a/t?client=te&format=html&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&dt=at&ie=UTF-8&oe=UTF-8&otf=2&ssel=0&tsel=0&kc=1"
...
def translate(source, source_lang="auto", dest_lang="en"):
    """Translate the source text from one language to another.
    return (result, original_language)
    """
    global BASE_URL, HEADERS
    data = {"q": source}
    url = "{url}&sl={source_lang}&tl={dest_lang}&hl={dest_lang}&tk={tk}".format(
        url=BASE_URL,
        source_lang=source_lang,
        dest_lang=dest_lang,
        tk=_calculate_tk(source),
    )
    response = _request(url, data=data)
    result = json.loads(response)[0]
    translation = result[0]
    original_language = result[1]
    return translation, original_language


def detect(source):
    """Detect the source text's language."""
    _, language = translate(source)
    return language
